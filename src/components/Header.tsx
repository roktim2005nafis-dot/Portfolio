...
export default function Header() {
  ...
  return (
    <motion.header 
      className={`fixed top-0 left-0 w-full z-50 transition-colors duration-500 ${isScrolled ? 'bg-[#050505]/80 backdrop-blur-md border-b border-white/5 py-4' : 'bg-transparent py-6 md:py-10'}`}
      initial={{ y: -100 }}
      animate={{ y: 0 }}
    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
        <a href="#" className="flex flex-col group" data-cursor="hover">
          ...
          <span className="font-display font-light text-xl tracking-tighter text-[#F5F5F5] uppercase">
        ...
        <nav className="hidden md:flex items-center gap-12">
          {['Projects', 'Expertise', 'Contact'].map((item) => (
            <a 
              ...
              className="text-[11px] uppercase tracking-[0.2em] text-[#F5F5F5]/60 hover:text-[#F5F5F5] transition-opacity"
              data-cursor="hover"
            >
              {item}
            </a>
          ...
          <div className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center">
            <div className="w-1 h-1 bg-[#F5F5F5] rounded-full"></div>
          </div>
        </nav>
